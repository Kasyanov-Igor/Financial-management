...
void Otchet(deque < Expenses>& massiv_Expenses)
{
	...
	case 1:
	{
		...
		try
		{
			dataUser.DataCheck(dataUser);
			Dayreport(massiv_Expenses, dataUser);
			
		}
		catch (int i)
		{
			string data;
			cout << "Not correct data:" << i << endl;
			cout << "cout data:" << endl;
			cin >> data;
			dataUser.data = data;
			Dayreport(massiv_Expenses, dataUser);
	...
	case 2:
	{
		cout << "Введите месяц и год для отчета (пример : 01.12.2022)" << endl;
		string d;
		cin >> d;
		Data dataUser(d);
		try
		{
			dataUser.DataCheck(dataUser);
			Monthreport(massiv_Expenses, dataUser);
	...
	case 3:
	{
		...
		{
			dataUser.DataCheck(dataUser);
			Weekreport(massiv_Expenses, dataUser);
		}
		catch (int i)
		{
			string data;
